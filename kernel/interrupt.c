...
Gate idt[IDT_LEN];
int intNestingLevels;
InterruptHandler irqHandlerTable[IRQ_COUNT];
void* sysCallTable[];
...
{
    /* 初始化中断向量表 */
    memset(idt, 0, sizeof(Gate) * gateCount);
    //预定义的内部中断
...
{
    sysCallTable[callNum] = handler;
}

void setupSystemCall()
{
    initGate(idt + INT_VECTOR_SYSTEM_CALL, DA_386IGate, systemCallInterrupt, PRIVILEGE_USER);
    setupTimeSystemCall();
}
